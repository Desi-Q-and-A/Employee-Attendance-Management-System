		...
		Optional<TaskModel> findTask =  taskRepository.findById(taskId);
		  if (findTask.isPresent()) {
			  TaskModel exTask = findTask.get();
		        ...
		        if (data.getTaskActions() == TaskActions.START   && exTask.getStartedAt() == null && exTask.getTaskStatus() == TaskStatus.PENDING) {
		            
		            LocalDateTime startDateTime = LocalDateTime.now();
		            exTask.setTaskActions(TaskActions.STARTED);
		            exTask.setStartedAt(startDateTime);
		            exTask.setTaskStatus(TaskStatus.IN_PROGRESS);
		        } else  if (data.getTaskActions() == TaskActions.PAUSE   && exTask.getStartedAt() != null && exTask.getTaskStatus() == TaskStatus.IN_PROGRESS) {
		        	 exTask.setTaskActions(TaskActions.PAUSED);
		            LocalDateTime startDateTime = LocalDateTime.now();
		            Duration duration = Duration.between(exTask.getStartedAt(), startDateTime);
		            exTask.setTimeTaken(duration);
		            exTask.setTaskStatus(TaskStatus.IN_PROGRESS);
		        } else  if (data.getTaskActions() == TaskActions.RESUME   && exTask.getStartedAt() != null  && exTask.getTaskStatus() == TaskStatus.IN_PROGRESS) {
		            ...
		            exTask.setTimeTaken(duration);
		            exTask.setTaskStatus(TaskStatus.IN_PROGRESS);
		        } else  if (data.getTaskActions() == TaskActions.END   && exTask.getStartedAt() != null  && exTask.getTaskStatus() == TaskStatus.IN_PROGRESS) {
